...
export default function HomePage() {
  ...
  const [showPassword, setShowPassword] = useState<boolean>(false); // 👁️ Göz İkonu Şalteri
      ...
      <div className="w-full max-w-4xl">
        ...
        <div className={`${activeTab === "escrow" ? "block" : "hidden"} bg-slate-900 border border-slate-800 p-6 sm:p-8 rounded-3xl shadow-xl`}>
          ...
          <div className="space-y-4">
            ...
            {/* 👁️ GÖZ İKONLU GÜVENLİK PAROLASI KUTUSU */}
            <div>
              ...
              <div className="relative">
                <input id="escrowPasswordInput" type={showPassword ? "text" : "password"} placeholder="Kilidi açacak gizli şifre belirleyin" value={escrowPassword} onChange={(e) => setEscrowPassword(e.target.value)} className="w-full p-3.5 pr-12 bg-slate-950 border border-emerald-900 rounded-xl text-sm text-white outline-none focus:border-emerald-500 placeholder-emerald-800 font-mono" />
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-gray-400 hover:text-emerald-400 transition-colors text-base font-normal flex items-center justify-center p-1">{showPassword ? "👁️" : "🙈"}</button>
              </div>
            </div>
